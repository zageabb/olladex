...
type ModelProfile = { id: number; name: string; chat_model: string; embedding_model: string; temperature: number; max_steps: number; context_files: number; context_chars: number };
...
export function ProjectPanel({ project, onUpdated }: { project: Project; onUpdated: (project: Project) => void }) {
  ...
  const [profileChat, setProfileChat] = useState(project.profile_chat_model || project.model);
  ...
  async function saveProfile() {
    if (!profileName.trim() || !profileChat.trim()) return;
    try {
      const payload = JSON.stringify({ name: profileName.trim(), chat_model: profileChat.trim(), embedding_model: profileEmbedding.trim(), temperature: profileTemperature, max_steps: profileSteps, context_files: profileFiles, context_chars: profileChars });
      const saved = await request<ModelProfile>(editingProfileId ? `/model-profiles/${editingProfileId}` : "/model-profiles", { method: editingProfileId ? "PATCH" : "POST", body: payload });
      setProfiles((items) => [...items.filter((item) => item.id !== saved.id), saved].sort((a, b) => a.name.localeCompare(b.name))); setProfileId(String(saved.id)); setEditingProfileId(saved.id); setNotice(editingProfileId ? "Model profile updated" : "Model profile created — save project settings to apply it");
    } catch (error) { setNotice(error instanceof Error ? error.message : String(error)); }
  }

  function editSelectedProfile() {
    const selected = profiles.find((profile) => profile.id === Number(profileId)); if (!selected) return;
    setEditingProfileId(selected.id); setProfileName(selected.name); setProfileChat(selected.chat_model); setProfileEmbedding(selected.embedding_model); setProfileTemperature(selected.temperature); setProfileSteps(selected.max_steps); setProfileFiles(selected.context_files); setProfileChars(selected.context_chars);
  }

  function newProfile() {
    setEditingProfileId(null); setProfileName(""); setProfileChat(model); setProfileEmbedding(project.profile_embedding_model || "nomic-embed-text"); setProfileTemperature(0.2); setProfileSteps(8); setProfileFiles(8); setProfileChars(32000);
  }

  async function deleteProfile() {
    if (!editingProfileId) return;
    try {
      await request(`/model-profiles/${editingProfileId}`, { method: "DELETE" });
      setProfiles((items) => items.filter((item) => item.id !== editingProfileId)); if (profileId === String(editingProfileId)) setProfileId(""); newProfile(); setNotice("Model profile deleted");
    } catch (error) { setNotice(error instanceof Error ? error.message : String(error)); }
  }
  ...
  return <div className="project-panel">
    ...
    <div className="project-columns">
      ...
      <section><p className="eyebrow">Agent configuration</p><h3>Project rules</h3><form onSubmit={save}><label>Model profile<select value={profileId} onChange={(e) => setProfileId(e.target.value)}><option value="">Project model only</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name} · {profile.chat_model}</option>)}</select></label><div className="profile-actions"><button type="button" onClick={editSelectedProfile} disabled={!profileId}>Edit selected</button><button type="button" onClick={newProfile}>New profile</button></div><label>Ollama model<input value={model} onChange={(e) => setModel(e.target.value)} /></label><details className="profile-builder"><summary>{editingProfileId ? "Edit reusable profile" : "Create reusable profile"}</summary><div><label>Profile name<input value={profileName} onChange={(e) => setProfileName(e.target.value)} placeholder="My coding profile" /></label><label>Chat model<input value={profileChat} onChange={(e) => setProfileChat(e.target.value)} /></label></div><div><label>Embedding model<input value={profileEmbedding} onChange={(e) => setProfileEmbedding(e.target.value)} /></label><label>Temperature<input type="number" min="0" max="2" step="0.05" value={profileTemperature} onChange={(e) => setProfileTemperature(Number(e.target.value))} /></label></div><div><label>Tool steps<input type="number" min="1" max="30" value={profileSteps} onChange={(e) => setProfileSteps(Number(e.target.value))} /></label><label>Context files<input type="number" min="1" max="30" value={profileFiles} onChange={(e) => setProfileFiles(Number(e.target.value))} /></label><label>Context characters<input type="number" min="4000" max="200000" step="1000" value={profileChars} onChange={(e) => setProfileChars(Number(e.target.value))} /></label></div><div className="profile-save-actions"><button type="button" onClick={saveProfile}>{editingProfileId ? "Update profile" : "Create profile"}</button>{editingProfileId && <button type="button" onClick={deleteProfile}>Delete</button>}</div></details><label>Approval mode<select value={mode} onChange={(e) => setMode(e.target.value as typeof mode)}><option value="review">Review — approve every command</option><option value="assisted">Assisted — safe checks run automatically</option><option value="autonomous">Autonomous — commands run automatically</option></select></label><label>Instructions<textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder="Use Python 3.12. Run tests before completion. Never modify deployment secrets…" /></label><div className="git-identity"><label>Git author<input value={gitName} onChange={(e) => setGitName(e.target.value)} /></label><label>Git email<input type="email" value={gitEmail} onChange={(e) => setGitEmail(e.target.value)} /></label></div><div><button className="primary">Save settings</button><span>{notice}</span></div></form></section>
    </div>
