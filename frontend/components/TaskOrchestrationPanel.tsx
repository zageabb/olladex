...
export function TaskOrchestrationPanel({ projectId, onCreated }: { projectId: number; onCreated: () => void }) {
  ...
  async function createTask(event: FormEvent) {
    ...
    try {
      await request(`/projects/${projectId}/orchestration/tasks`, {
        method: "POST",
        body: JSON.stringify({
          title: draft.title.trim(), prompt: draft.prompt.trim(), agent_role: draft.role.trim() || "worker",
          parent_task_id: draft.parent ? Number(draft.parent) : null, depends_on: dependencies,
        }),
      });
  ...
  function renderNode(node: Node, depth = 0): React.ReactNode {
    return <div key={node.id} className="orchestration-node" style={{ marginLeft: depth * 16 }}>
      <div className="orchestration-node-head"><div><strong>#{node.id} {node.title}</strong><small>{node.agent_role} · {node.status}{node.worktree_branch ? ` · ${node.worktree_branch}` : ""}{node.pr_number ? ` · PR #${node.pr_number} ${node.pr_state || ""}` : ""}</small></div><span>{node.children.length} child{node.children.length === 1 ? "" : "ren"}</span></div>
  ...
  return <section className="orchestration-panel">
    ...
    <div className="orchestration-grid">
      ...
      <form onSubmit={createTask} className="orchestration-form">
        ...
        <button className="primary" disabled={busy || !draft.prompt.trim()}>{busy ? "Queueing…" : "Queue orchestrated task"}</button>
      </form>
    </div>
