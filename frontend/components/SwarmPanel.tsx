...
type Agent = { id:number; title:string; status:string; agent_role:string; assigned_model:string; task_kind:string; priority:number; progress:number; current_activity:string; depends_on?:number[]|string; run_id?:number|null; worktree_branch?:string; result?:string; error?:string; created_at:string; started_at?:string; tool_usage?:number; tool_budget?:number; latest_insight?:{category:string;content:string;created_at:string}|null };
type SwarmRun = { id:number; project_id:number; title:string; objective:string; status:string; profile_id:number; max_agents:number; max_concurrency:number; total_agents_created:number; created_at:string; started_at:string; completed_at:string; agents?:Agent[]; agent_counts?:Record<string,number>; coordinator_activity?:{category:string;key:string;content:string;created_at:string}|null; coordinator_budget?:{used:number;budget:number;remaining:number}; integration_path?:string; integration_branch?:string; integration_check_command?:string; integration_check_status?:string; integration_check_output?:string; integration_pr_number?:number; integration_pr_url?:string; integration_pr_state?:string };
      ...
      {profiles.find(item=>String(item.id)===profileId)&&<details className={styles.roleSettings}>
        ...
        {(()=>{
          ...
          return <div className={styles.roleGrid}>
            ...
            <label>Worker tool budget<input type="number" min="1" max="200" value={profile.agent_tool_budget} onChange={event=>setProfiles(items=>items.map(item=>item.id===profile.id?{...item,agent_tool_budget:Number(event.target.value)}:item))}/></label>
            <label>Coordinator budget<input type="number" min="1" max="200" value={profile.coordinator_tool_budget} onChange={event=>setProfiles(items=>items.map(item=>item.id===profile.id?{...item,coordinator_tool_budget:Number(event.target.value)}:item))}/></label>
            <label><span>Dynamic swarm size</span><input type="checkbox" checked={Boolean(profile.dynamic_size)} onChange={event=>setProfiles(items=>items.map(item=>item.id===profile.id?{...item,dynamic_size:event.target.checked?1:0}:item))}/></label>
        ...
        <section>
          ...
          <div className={styles.agents}>
            {agents.map(agent=><article key={agent.id} className={styles.agent}>
              ...
              <h4>{agent.title}</h4>
              <dl><div><dt>Model</dt><dd>{agent.assigned_model||"Project default"}</dd></div><div><dt>Branch</dt><dd>{agent.worktree_branch||"waiting"}</dd></div><div><dt>Run</dt><dd>{agent.run_id?"#"+agent.run_id:"not started"}</dd></div><div><dt>Progress</dt><dd>{agent.status==="completed"?100:Number(agent.progress||0)}%</dd></div><div><dt>Tools</dt><dd>{Number(agent.tool_usage||0)}{agent.tool_budget?"/"+agent.tool_budget:""}</dd></div></dl>
              <p>{agent.current_activity||agent.result||agent.error||"Waiting for activity…"}</p>
              {agent.latest_insight&&<aside className={styles.insight}><b>{agent.latest_insight.category}</b><span>{agent.latest_insight.content}</span></aside>}
