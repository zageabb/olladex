...
- Persistent background agent queue with configurable parallel workers and visible queued, running, paused, completed, failed and cancelled states.
- Cooperative pause, resume and cancellation of in-flight agent turns between model and tool steps.
...
- Pausing or cancelling a running background job takes effect after its current Ollama HTTP request returns; the next model or tool step does not start.

## Next release direction

The queue concurrency and pause/resume foundations are now implemented. The remaining v0.7 direction is native Windows ConPTY, issue comments/status updates, authenticated private-release updates and visual browser testing.
